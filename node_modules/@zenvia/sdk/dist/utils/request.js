...
async function post(token, path, body, logger, formData) {
    return request(token, 'post', path, body, logger, formData);
}
exports.post = post;
async function get(token, path, logger) {
    return request(token, 'get', path, undefined, logger);
}
exports.get = get;
async function patch(token, path, body, logger) {
    return request(token, 'patch', path, body, logger);
}
exports.patch = patch;
async function del(token, path, logger) {
    return request(token, 'delete', path, undefined, logger);
}
exports.del = del;
async function request(token, method, path, body, logger, formData) {
    const uri = `${url}${path}`;
    const data = {
        method,
        uri,
